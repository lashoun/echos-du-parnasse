...
export default async function AdminCollectionsPage({
  ...
  const { data: collections } = await supabase
    .from('collections')
    .select('id, title, year, author_id, authors(name)')
    .order('title')
  ...
  return (
    <div>
      ...
      {!collections || collections.length === 0 ? (
        <p className="text-stone-500">Aucune collection pour le moment.</p>
      ) : (
              ...
              {collections.map((col) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-stone-600 dark:text-stone-400">
                    {(col.authors as unknown as { name: string } | null)
                      ?.name ?? '—'}
                  </td>
                  <td className="px-4 py-3 text-stone-600 dark:text-stone-400">
                    {col.year ?? '—'}
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <ConfirmDeleteForm
                        ...
                        fieldValue={col.id}
                        confirmMessage={`Supprimer la collection « ${col.title} » ?`}
                      >
                        <button
                          type="submit"
